...
export class WebAccount {
    ...
    constructor(public readonly wallet: WalletContextState) {
        this.fakeAccount = new AccountImpl(HexBuffer(wallet.account!.address!.toString()))
    }
    ...
    async rawToSigned(txn: Transaction): Promise<TxnBuilderTypes.SignedTransaction> {
        const signingMessage = TransactionBuilder.getSigningMessage(txn.raw);
        const signature = await this.signFn(signingMessage);

        const authenticator = new TransactionAuthenticatorEd25519(new Ed25519PublicKey(this.publicKeyBytes()), signature as TxnBuilderTypes.Ed25519Signature);
    ...
    async signFn(message: TxnBuilderTypes.SigningMessage) {
        console.log("signFn...")
        const sig = (await this.wallet.signMessage(Buffer.from(message).toString())) as any as {signature?:string, result?:{hexString:string}};
        console.log("---sig:", sig);
        return this.fakeAccount.signFn(message);
        return new TxnBuilderTypes.Ed25519Signature(HexBuffer(sig.signature || sig.result!.hexString).reverse());
    }

    async getSigData(txn: Transaction): Promise<[signing: TxnBuilderTypes.SigningMessage, signature: TxnBuilderTypes.Ed25519Signature[]]> {
    ...
    async sendMultisigTx(payload: string, public_keys: string[], noncePubKey:TxnBuilderTypes.Ed25519PublicKey, signatures:{key:string, value:string}[], threshold: number) {
        console.log(payload);
        console.log(public_keys);
        console.log(noncePubKey.value);
        console.log(signatures);
        console.log(threshold);
        const fullPubkeys = public_keys.map(hexKey => new Ed25519PublicKey(HexBuffer(hexKey)));
        ...
        signatures.forEach(({key: pubkey, value: signature}) => {
            ...
            signaturesUnsorted.push([pk_index, new TxnBuilderTypes.Ed25519Signature(HexBuffer(signature))]);
        });
        console.log('bitmap:', bitmap);
        const signaturesSorted = signaturesUnsorted.sort((a, b) => a[0] - b[0]).map(v => v[1]);
        ...
        const finalSignature = new TxnBuilderTypes.MultiEd25519Signature(
            signaturesSorted, parsedBitmap,
        );
        const finalPubkeys = new TxnBuilderTypes.MultiEd25519PublicKey(fullPubkeys, threshold);

        const authenticator = new TxnBuilderTypes.TransactionAuthenticatorMultiEd25519(finalPubkeys, finalSignature);
