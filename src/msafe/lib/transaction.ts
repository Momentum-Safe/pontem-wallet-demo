...
abstract class AptosTxnBuilder {
  ...
  private _validateAndFix() {
    if (this._fromAddress === undefined) {
      throw new Error(
        "When building transaction, from address must be specified"
      );
    }
    if (this._sequenceNumber === undefined) {
      throw new Error(
        "When building transaction, sequence number must be specified"
      );
    }
...
export class AptosCoinTransferTxnBuilder extends AptosTxnBuilder {
  ...
  validateAndFix() {
    if (this._toAddress === undefined) {
      throw new Error(
        "When building transaction, to address must be specified"
      );
    }
  ...
  payload(): TxnBuilderTypes.TransactionPayload {
    ...
    const scriptFn = TxnBuilderTypes.EntryFunction.natural(
      ...
      [
        BCS.bcsToBytes(
          TxnBuilderTypes.AccountAddress.fromHex(this._toAddress as HexString)
        ),
...
export class AptosModuleTxnBuilder extends AptosTxnBuilder {
  private _module!: HexString;

  module(_module: HexString | string): this {
    this._module =
      typeof _module === "string" ? new HexString(_module) : _module;
    return this;
  }
  ...
  validateAndFix() {
    if (this._module === undefined) {
      throw new Error(
        "When building transaction, hex module must be specified"
      );
    }
  }
  ...
  payload(): TxnBuilderTypes.TransactionPayload {
    return new TxnBuilderTypes.TransactionPayloadModuleBundle(
      new TxnBuilderTypes.ModuleBundle([
        new TxnBuilderTypes.Module(this._module.toUint8Array()),
      ])
    );
  }
}
...
export class AptosEntryTxnBuilder extends AptosTxnBuilder {
  ...
  validateAndFix() {
    if (this._module === undefined) {
      throw new Error(
        "When building transaction, module name must be specified"
      );
    }
    if (this._method === undefined) {
      throw new Error(
        "When building transaction, method name must be specified"
      );
    }
    if (this._contract === undefined) {
      throw new Error(
        "When building transaction, contract address must be specified"
      );
    }
  }
